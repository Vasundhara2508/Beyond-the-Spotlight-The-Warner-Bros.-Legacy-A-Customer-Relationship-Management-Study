...
section[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #0E0E1A 0%, #16162A 100%) !important;
...
.card-blue  { border-left: 4px solid var(--accent1);}
...
.page-hero::before {
    ...
    background: radial-gradient(ellipse at 70% 50%, rgba(245,200,66,.07) 0%, transparent 60%);
}
.hero-title {
...
.hero-sub {
    ...
    margin-top: .4rem; letter-spacing: 1px; text-transform: uppercase;
}
.hero-tag {
...
TEXT   = "#E8E8F0"
BORDER = "#2A2A3E"
PALETTE = [GOLD, RED, BLUE, GREEN, PURPLE, "#F97316", "#06B6D4", "#EC4899"]
...
def load_data():
    ...
    wbd["Date"] = pd.to_datetime(wbd["Date"], format="mixed", dayfirst=True, errors="coerce")
    wbd = wbd.sort_values("Date")

    # ── Warner Bros movies ─────────────────────────
    movies = pd.read_csv("tmdb_5000_movies.csv")
    movies = movies[['title','release_date','revenue','budget',
                     'popularity','vote_average','production_companies']]
    movies = movies[movies['production_companies'].str.contains("Warner", na=False)]
    movies = movies[movies['revenue'] > 0]
    movies['release_year'] = pd.to_datetime(
        movies['release_date'], errors='coerce').dt.year
...
if section == "Overview":
    ...
    with c1:
        st.metric("Stock Price", f"${wbd['Close'].iloc[-1]:.2f}",
                  f"{wbd['Close'].iloc[-1]-wbd['Close'].iloc[-2]:+.2f}")
    with c2: st.metric("Total WB Movies", str(len(movies)))
    with c3: st.metric("Churn Rate",      f"{netflix['churned'].mean()*100:.1f}%")
    with c4: st.metric("Avg Revenue",     f"${movies['revenue'].mean()/1e6:.0f}M")

    st.markdown("---")
    col_l, col_r = st.columns(2)

    with col_l:
        st.markdown('<div class="section-header">Stock Closing Price</div>', unsafe_allow_html=True)
        fig, ax = plt.subplots(figsize=(6, 3))
        ax.plot(wbd["Date"], wbd["Close"], color=GOLD, lw=1.5)
        ax.fill_between(wbd["Date"], wbd["Close"], alpha=.08, color=GOLD)
        ax.set_xlabel("Year"); ax.set_ylabel("USD")
        ax.set_title("WBD Closing Price")
        apply_theme(fig, [ax]); fig_list.append(fig);st.pyplot(fig); plt.close()

    with col_r:
        st.markdown('<div class="section-header">Top 5 Franchise Revenue</div>', unsafe_allow_html=True)
        top5 = movies.nlargest(5, "revenue")
        fig, ax = plt.subplots(figsize=(6, 3))
        ax.barh(top5["title"], top5["revenue"]/1e6,
                color=PALETTE[:5], edgecolor="none", height=.55)
    ...
    for col, title, desc, color in [
        (m1,  "Stock Analysis", "Price, Volume & Volatility trends for WBD equity", "card-gold"),
        (m2, "Franchise",      "Revenue, ratings & audience metrics for WB films",  "card-red"),
        (m3,  "Churn Model",    "ML models predicting Netflix subscriber churn",      "card-blue"),
...
if section == "Stock Analysis":
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.metric("Latest Close",   f"${wbd['Close'].iloc[-1]:.2f}")
    with c2: st.metric("52-Wk High",     f"${wbd['Close'].max():.2f}")
    with c3: st.metric("52-Wk Low",      f"${wbd['Close'].min():.2f}")
    with c4: st.metric("Avg Volatility", f"{wbd['Volatility'].mean():.2f}")
    ...
    ax.fill_between(wbd["Date"], wbd["Close"], alpha=.1, color=GOLD)
    ax.set_title("Warner Bros Discovery — Stock Closing Price")
    ax.set_xlabel("Year"); ax.set_ylabel("Stock Price (USD)")
    apply_theme(fig, [ax]); st.pyplot(fig); plt.close()
    ...
    with col1:
        ...
        ax.fill_between(wbd["Date"], wbd["Volume"], alpha=.1, color=BLUE)
        ax.set_title("Trading Volume Over Time")
        ax.set_xlabel("Year"); ax.set_ylabel("Volume")
        apply_theme(fig, [ax]); st.pyplot(fig); plt.close()
    ...
    with col2:
        ...
        fig, ax = plt.subplots(figsize=(7, 3.5))
        ax.plot(wbd["Date"], wbd["Close"],  color=TEXT,  lw=1.0, alpha=.5, label="Close Price")
    ...
    # ── G4  Volatility ─────────────────────────────────
    with col3:
        st.markdown('<div class="section-header">Stock Volatility</div>', unsafe_allow_html=True)
        fig, ax = plt.subplots(figsize=(7, 3.5))
        ax.plot(wbd["Date"], wbd["Volatility"], color=PURPLE, lw=1.3)
        ax.fill_between(wbd["Date"], wbd["Volatility"], alpha=.15, color=PURPLE)
    ...
    with col4:
        ...
        fig, ax = plt.subplots(figsize=(7, 3.5))
        sns.histplot(wbd["Close"], bins=30, kde=True, ax=ax,
                     color=GOLD, edgecolor=PANEL,
                     line_kws={"color": RED, "lw": 2})
        ax.set_title("Distribution of WBD Stock Prices")
        ax.set_xlabel("Stock Price"); ax.set_ylabel("Frequency")
        apply_theme(fig, [ax]); fig_list.append(fig); st.pyplot(fig); plt.close()

    # ── G6  Correlation Heatmap ─────────────────────────
    st.markdown('<div class="section-header">Correlation Heatmap</div>', unsafe_allow_html=True)
    fig, ax = plt.subplots(figsize=(8, 4))
    sns.heatmap(wbd.corr(numeric_only=True), annot=True, fmt=".2f",
                cmap="coolwarm", ax=ax,
                linewidths=.4, linecolor=BORDER,
                annot_kws={"size": 9})
...
if section == "Franchise":
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.metric("Total Films",    len(movies))
    with c2: st.metric("Top Revenue",    f"${movies['revenue'].max()/1e6:.0f}M")
    with c3: st.metric("Avg Rating",     f"{movies['vote_average'].mean():.1f} / 10")
    with c4: st.metric("Avg Popularity", f"{movies['popularity'].mean():.1f}")
    ...
    with col1:
        ...
        bars = ax.barh(top10["title"], top10["revenue"]/1e6,
                       color=PALETTE[:len(top10)], edgecolor="none", height=.6)
        ax.set_xlabel("Revenue (USD M)")
        ax.set_title("Top Warner Bros Movies by Box Office Revenue")
        for bar, val in zip(bars, top10["revenue"]/1e6):
            ax.text(val + 2, bar.get_y()+bar.get_height()/2,
                    f"${val:.0f}M", va="center", fontsize=7.5, color=TEXT)
        apply_theme(fig, [ax]);fig_list.append(fig); st.pyplot(fig); plt.close()

    # ── G2  Revenue Trend ──────────────────────────────
    with col2:
        st.markdown('<div class="section-header">Revenue Trend Over Years</div>', unsafe_allow_html=True)
        trend = movies.groupby("release_year")["revenue"].mean().reset_index()
        fig, ax = plt.subplots(figsize=(7, 5))
        ax.plot(trend["release_year"], trend["revenue"]/1e6,
                color=GOLD, lw=2, marker="o", ms=4)
        ax.fill_between(trend["release_year"], trend["revenue"]/1e6,
                        alpha=.1, color=GOLD)
        ax.set_title("Warner Bros Revenue Trend Over Years")
        ax.set_xlabel("Release Year"); ax.set_ylabel("Avg Revenue (USD M)")
        apply_theme(fig, [ax]); fig_list.append(fig); st.pyplot(fig); plt.close()
    ...
    with col3:
        st.markdown('<div class="section-header">Popularity vs Revenue</div>', unsafe_allow_html=True)
        fig, ax = plt.subplots(figsize=(7, 4))
        scatter = ax.scatter(movies["popularity"], movies["revenue"]/1e6,
                             c=movies["vote_average"], cmap="plasma",
                             s=60, alpha=.8, edgecolors=PANEL, linewidths=.5)
        ...
        ax.set_xlabel("Popularity Score"); ax.set_ylabel("Revenue (USD M)")
        apply_theme(fig, [ax]); fig_list.append(fig);st.pyplot(fig); plt.close()

    # ── G4  Rating Distribution ────────────────────────
    with col4:
        st.markdown('<div class="section-header">Movie Rating Distribution</div>', unsafe_allow_html=True)
        fig, ax = plt.subplots(figsize=(7, 4))
        sns.histplot(movies["vote_average"], bins=20, kde=True, ax=ax,
                     color=GREEN, edgecolor=PANEL,
                     line_kws={"color": GOLD, "lw": 2})
        ax.axvline(movies["vote_average"].mean(), color=RED, ls="--", lw=1.5,
...
if section == "Churn Model":
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.metric("Churn Rate",         f"{y.mean()*100:.1f}%")
    with c2: st.metric("Logistic Reg Acc",   f"{log_acc*100:.1f}%")
    with c3: st.metric("Random Forest Acc",  f"{rf_acc*100:.1f}%")
    with c4: st.metric("XGBoost Acc",        f"{xgb_acc*100:.1f}%")
    ...
    with col1:
        ...
        bars = ax.bar(results["Model"], results["Accuracy"]*100,
                      color=[RED, GOLD, GREEN], edgecolor="none", width=.5)
        ax.set_ylim(0, 110)
    ...
    with col2:
        ...
        ax.barh(fi["Feature"], fi["Importance"],
                color=PALETTE[:len(fi)], edgecolor="none", height=.55)
...
if section == "Survey":
    ...
    # ── BRAND FAMILIARITY ──────────────────────────────
    with col1:
        st.markdown('<div class="section-header">Brand Familiarity</div>', unsafe_allow_html=True)
        fam = survey[Q_FAM].value_counts()

        fig, ax = plt.subplots(figsize=(10, 8))
        bars = ax.barh(fam.index, fam.values / fam.sum() * 100,
                       color=PALETTE[:len(fam)], edgecolor="none")
    ...
    with col2:
        st.markdown('<div class="section-header">Popularity vs Rivals</div>', unsafe_allow_html=True)
    ...
    with col3:
        st.markdown('<div class="section-header">Personalization Perception</div>', unsafe_allow_html=True)

        pers = survey[Q_PERS].value_counts()

        fig, ax = plt.subplots(figsize=(6, 4))
        bars = ax.bar(pers.index, pers.values / pers.sum() * 100,
                      color=PALETTE[:len(pers)], edgecolor="none")
    ...
    with col4:
        st.markdown('<div class="section-header">Long-term Relationship Builder</div>', unsafe_allow_html=True)

        rel = survey[Q_REL].value_counts()

        fig, ax = plt.subplots(figsize=(6, 4))
        bars = ax.bar(rel.index, rel.values / rel.sum() * 100,
                      color=PALETTE[:len(rel)], edgecolor="none")
    ...
    # ── CROSS TAB HEATMAP ──────────────────────────────
    st.markdown('<div class="section-header">Familiarity vs Popularity</div>', unsafe_allow_html=True)
    ...
    sns.heatmap(cross_tab, annot=True, fmt=".1f",
                cmap="YlOrRd",
                linewidths=.3)
    ...
    st.markdown('<div class="section-header">CRM Gap Analysis</div>', unsafe_allow_html=True)

    crm_gap = pd.DataFrame({
        "CRM Feature": ["Personalization", "Churn Prediction", "Customer Data", "Recommendations"],
        "Netflix": ["Strong AI", "Predictive Models", "Unified Data", "Advanced ML"],
